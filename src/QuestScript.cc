...
struct QuestScriptOpcodeDefinition {
  ...
  uint16_t opcode;
  // By convention, the first of these names is the canonical name (used when disassembling) and the last is the Qedit
  // name. When assembling, all names in this list are considered valid.
  std::vector<const char*> names;
  std::vector<Argument> args;
  uint32_t flags;

  std::string str() const {
    return std::format("{:04X}: {} flags={:04X}", this->opcode, phosg::join(this->names, "/"), this->flags);
  }
};
...
static const QuestScriptOpcodeDefinition opcode_defs[] = {
    ...
    {0x05, {"va_start"}, {}, F_V3_V4 | F_CLEAR_ARGS},
    ...
    {0x07, {"va_call"}, {SCRIPT16}, F_V3_V4 | F_CLEAR_ARGS},

    // Copies a value from regB to regA
    {0x08, {"let"}, {W_REG, R_REG}, F_V0_V4},

    // Sets regA to valueB
    {0x09, {"leti"}, {W_REG, I32}, F_V0_V4},
    ...
    {0x15, {"grev"}, {I16}, F_V0_V4},

    // If regB is nonzero, sets flagA; otherwise, clears it
    {0x16, {"glet"}, {I16, R_REG}, F_V0_V4},

    // Sets regB to the value of flagA
    {0x17, {"gget"}, {I16, R_REG}, F_V0_V4},

    // regA += regB
    {0x18, {"add"}, {W_REG, R_REG}, F_V0_V4},

    // regA += valueB
    {0x19, {"addi"}, {W_REG, I32}, F_V0_V4},

    // regA -= regB
    {0x1A, {"sub"}, {W_REG, R_REG}, F_V0_V4},

    // regA -= valueB
    {0x1B, {"subi"}, {W_REG, I32}, F_V0_V4},

    // regA *= regB
    {0x1C, {"mul"}, {W_REG, R_REG}, F_V0_V4},

    // regA *= valueB
    {0x1D, {"muli"}, {W_REG, I32}, F_V0_V4},

    // regA /= regB
    {0x1E, {"div"}, {W_REG, R_REG}, F_V0_V4},

    // regA /= valueB
    {0x1F, {"divi"}, {W_REG, I32}, F_V0_V4},

    // regA &= regB
    {0x20, {"and"}, {W_REG, R_REG}, F_V0_V4},

    // regA &= valueB
    {0x21, {"andi"}, {W_REG, I32}, F_V0_V4},

    // regA |= regB
    {0x22, {"or"}, {W_REG, R_REG}, F_V0_V4},

    // regA |= valueB
    {0x23, {"ori"}, {W_REG, I32}, F_V0_V4},

    // regA ^= regB
    {0x24, {"xor"}, {W_REG, R_REG}, F_V0_V4},

    // regA ^= valueB
    {0x25, {"xori"}, {W_REG, I32}, F_V0_V4},

    // regA %= regB
    // Note: This does signed division, so if the value is negative, you might get unexpected results.
    {0x26, {"mod"}, {W_REG, R_REG}, F_V3_V4},
    ...
    // called does so.
    {0x29, {"call"}, {SCRIPT16}, F_V0_V4 | F_CLEAR_ARGS},

    // If all values in regsB are nonzero, jumps to labelA
    {0x2A, {"jmp_on"}, {SCRIPT16, R_REG_SET}, F_V0_V4},

    // If all values in regsB are zero, jumps to labelA
    {0x2B, {"jmp_off"}, {SCRIPT16, R_REG_SET}, F_V0_V4},
    ...
    {0x4C, {"arg_pusha"}, {R_REG}, F_V3_V4 | F_PUSH_ARG},
    ...
    {0x61, {"npc_stop"}, {I32}, F_V0_V4 | F_ARGS},

    // Tells an NPC (by client ID) to follow the player
    {0x62, {"npc_play"}, {I32}, F_V0_V4 | F_ARGS},

    // Destroys an NPC (by client ID)
    {0x63, {"npc_kill"}, {I32}, F_V0_V4 | F_ARGS},
    ...
    {0x68, {"create_pipe"}, {I32}, F_V0_V4 | F_ARGS},
    ...
    {0x6E, {"p_look"}, {CLIENT_ID}, F_V0_V4 | F_ARGS},

    // Disables/enables attacks for all players
    {0x70, {"p_action_disable"}, {}, F_V0_V4},
    {0x71, {"p_action_enable"}, {}, F_V0_V4},

    // Disables/enables movement for the given player
    {0x72, {"disable_movement1"}, {CLIENT_ID}, F_V0_V4 | F_ARGS},
    {0x73, {"enable_movement1"}, {CLIENT_ID}, F_V0_V4 | F_ARGS},
    ...
    {0x7A, {"npc_talk_kill"}, {I32}, F_V0_V4 | F_ARGS},
    ...
    {0x7E, {"p_look_at"}, {CLIENT_ID, CLIENT_ID}, F_V0_V4 | F_ARGS},
    ...
    {0x80, {"cam_quake"}, {}, F_V0_V4},

    // Moves the camera to where your character is looking.
    {0x81, {"cam_adj"}, {}, F_V0_V4},

    // Zooms the camera in or out.
    {0x82, {"cam_zmin"}, {}, F_V0_V4},
    {0x83, {"cam_zmout"}, {}, F_V0_V4},
    ...
    {0x8C, {"at_coords_call"}, {{R_REG_SET_FIXED, 5}}, F_V0_V4},
    ...
    {0x8D, {"at_coords_talk"}, {{R_REG_SET_FIXED, 5}}, F_V0_V4},
    ...
    {0x90, {"switch_on"}, {I32}, F_V0_V4 | F_ARGS},
    {0x91, {"switch_off"}, {I32}, F_V0_V4 | F_ARGS},
    ...
    {0x92, {"playbgm_epi"}, {I32}, F_V0_V4 | F_ARGS},
    ...
    {0x93, {"set_mainwarp"}, {I32}, F_V0_V4 | F_ARGS},
    ...
    {0x94, {"set_obj_param"}, {{R_REG_SET_FIXED, 6}, W_REG}, F_V0_V4},
    ...
    {0x95, {"set_floor_handler"}, {FLOOR, SCRIPT32}, F_V0_V2},
    {0x95, {"set_floor_handler"}, {FLOOR, SCRIPT16}, F_V3_V4 | F_ARGS},

    // Deletes the handler for floorA.
    {0x96, {"clr_floor_handler"}, {FLOOR}, F_V0_V4 | F_ARGS},
    ...
    {0x98, {"hud_hide"}, {}, F_V0_V4},
    {0x99, {"hud_show"}, {}, F_V0_V4},

    // Enables/disables the cinema effect (black bars above/below screen)
    {0x9A, {"cine_enable"}, {}, F_V0_V4},
    {0x9B, {"cine_disable"}, {}, F_V0_V4},
    ...
    // this function).
    {0xA1, {"set_qt_failure"}, {SCRIPT32}, F_V0_V2},
    {0xA1, {"set_qt_failure"}, {SCRIPT16}, F_V3_V4},
    ...
    {0xB0, {"pl_add_meseta"}, {CLIENT_ID, I32}, F_V0_V4 | F_ARGS},

    // Starts a new thread at labelA in the quest script.
    {0xB1, {"thread_stg"}, {SCRIPT16}, F_V0_V4},
    ...
    {0xB2, {"del_obj_param"}, {R_REG}, F_V0_V4},
    ...
    {0xB3, {"item_create"}, {{R_REG_SET_FIXED, 3}, W_REG}, F_V0_V4},

    // Like item_create, but regsA specify all of item.data1 instead of only the first 3 bytes.
    {0xB4, {"item_create2"}, {{R_REG_SET_FIXED, 12}, W_REG}, F_V0_V4},
    ...
    {0xB5, {"item_delete"}, {R_REG, {W_REG_SET_FIXED, 12}}, F_V0_V4},
    ...
    {0xB8, {"setevt"}, {I32}, F_V05_V4 | F_ARGS},
    ...
    {0xBA, {"set_qt_exit"}, {SCRIPT32}, F_V05_V2},
    {0xBA, {"set_qt_exit"}, {SCRIPT16}, F_V3_V4},

    // Clears the quest exit handler (opposite of set_qt_exit).
    {0xBB, {"clr_qt_exit"}, {}, F_V05_V4},
    ...
    //   regsA[3] = ignored
    {0xC4, {"map_designate"}, {{R_REG_SET_FIXED, 4}}, F_V05_V4},

    // Locks (masterkey_on) or unlocks (masterkey_off) all doors
    {0xC5, {"masterkey_on"}, {}, F_V05_V4},
    {0xC6, {"masterkey_off"}, {}, F_V05_V4},
    ...
    {0xC9, {"winset_time"}, {R_REG}, F_V05_V4},
    ...
    {0xCB, {"set_quest_board_handler"}, {I32, SCRIPT32, CSTRING}, F_V05_V2},
    {0xCB, {"set_quest_board_handler"}, {I32, SCRIPT16, CSTRING}, F_V3_V4 | F_ARGS},

    // Deletes an item by index from the quest board.
    {0xCC, {"clear_quest_board_handler"}, {I32}, F_V05_V4 | F_ARGS},
    ...
    {0xD2, {"enable_mainmenu"}, {}, F_V1_V4},
    {0xD3, {"disable_mainmenu"}, {}, F_V1_V4},
    ...
    {0xD4, {"start_battlebgm"}, {}, F_V1_V4},
    {0xD5, {"end_battlebgm"}, {}, F_V1_V4},

    // Shows a message in the Quest Board message window
    {0xD6, {"disp_msg_qb"}, {CSTRING}, F_V1_V4 | F_ARGS},

    // Closes the Quest Board message window
    {0xD7, {"close_msg_qb"}, {}, F_V1_V4},
    ...
    // TODO: Document these
    {0xDA, {"set_returnhunter"}, {}, F_V1_V4},
    {0xDB, {"set_returncity"}, {}, F_V1_V4},

    // TODO: Document this
    {0xDC, {"load_pvr"}, {}, F_V1_V4},
    // TODO: Document this
    // Does nothing on all non-DC versions.
    {0xDD, {"load_midi"}, {}, F_V1_V4},
    ...
    {0xE4, {"warp_on"}, {}, F_V1_V4},
    {0xE5, {"warp_off"}, {}, F_V1_V4},
    ...
    {0xE8, {"set_eventflag2"}, {I32, R_REG}, F_V1_V4 | F_ARGS},
    ...
    {0xED, {"create_bgmctrl"}, {}, F_V1_V4},

    // Like pl_add_meseta, but can only give Meseta to the local player.
    {0xEE, {"pl_add_meseta2"}, {I32}, F_V1_V4 | F_ARGS},
    ...
    {0xEF, {"sync_register2"}, {W_REG, I32}, F_V3_V4 | F_ARGS},
    ...
    //   regB = client ID
    {0xF80A, {"get_coord_of_player"}, {{W_REG_SET_FIXED, 3}, R_REG}, F_V2_V4},
    ...
    {0xF810, {"ba_initial_floor"}, {I32}, F_V2_V4 | F_ARGS},
    ...
    //   1 => ALLOW
    //   2 => LIMIT_LIVES
    {0xF818, {"ba_set_respawn"}, {I32}, F_V2_V4 | F_ARGS},
    ...
    {0xF81A, {"ba_dropwep"}, {I32}, F_V2_V4 | F_ARGS},

    // Enables (1) or disables (0) teams in battle
    {0xF81B, {"ba_teams"}, {I32}, F_V2_V4 | F_ARGS},
    ...
    {0xF825, {"exp_multiplication"}, {{R_REG_SET_FIXED, 3}}, F_V2_V4},
    ...
    {0xF830, {"control_dragon"}, {R_REG}, F_V2_V4},
    ...
    {0xF838, {"shrink"}, {R_REG}, F_V2_V4},
    {0xF839, {"unshrink"}, {R_REG}, F_V2_V4},
    ...
    {0xF83A, {"set_shrink_cam1"}, {{R_REG_SET_FIXED, 4}}, F_V2_V4},
    {0xF83B, {"set_shrink_cam2"}, {{R_REG_SET_FIXED, 4}}, F_V2_V4},

    // Shows the timer window in challenge mode. regA is the time value to display, in seconds.
    {0xF83C, {"disp_time_cmode"}, {R_REG}, F_V2_V4},
    ...
    //   strC = custom name
    {0xF862, {"give_s_rank_weapon"}, {R_REG32, R_REG32, CSTRING}, F_V2},
    {0xF862, {"give_s_rank_weapon"}, {I32, R_REG, CSTRING}, F_V3_V4 | F_ARGS},
    ...
    {0xF863, {"get_mag_levels"}, {{W_REG32_SET_FIXED, 4}}, F_V2},
    {0xF863, {"get_mag_levels"}, {{W_REG_SET_FIXED, 4}}, F_V3_V4},
    ...
    {0xF869, {"check_rank_time"}, {R_REG, W_REG}, F_V2_V4},
    ...
    {0xF86A, {"item_create_cmode"}, {{R_REG_SET_FIXED, 6}, W_REG}, F_V2_V4},
    ...
    {0xF86F, {"ba_set_lives"}, {I32}, F_V2_V4 | F_ARGS},
    ...
    {0xF872, {"ba_set_time_limit"}, {I32}, F_V2_V4 | F_ARGS},
    ...
    {0xF875, {"enable_stealth_suit_effect"}, {R_REG}, F_V2_V4},
    {0xF876, {"disable_stealth_suit_effect"}, {R_REG}, F_V2_V4},

    // Enables or disables the use of techniques for a player. regA is the client ID.
    {0xF877, {"enable_techs"}, {R_REG}, F_V2_V4},
    {0xF878, {"disable_techs"}, {R_REG}, F_V2_V4},

    // Returns the gender of a character.
    //   regA = client ID
    //   regB = returned gender (0 = male, 1 = female, 2 = no player present or invalid class flags)
    {0xF879, {"get_gender"}, {R_REG, W_REG}, F_V2_V4},
    ...
    {0xF87A, {"get_chara_class"}, {R_REG, {W_REG_SET_FIXED, 2}}, F_V2_V4},
    ...
    {0xF87B, {"take_slot_meseta"}, {{R_REG_SET_FIXED, 2}, {W_REG_SET_FIXED, 2}}, F_V2_V4},
    ...
    {0xF87D, {"kill_player"}, {R_REG}, F_V2_V4},

    // Returns (in regA) the player's serial number. On BB, returns 0.
    {0xF87E, {"get_serial_number"}, {W_REG}, F_V2_V3},
    {0xF87E, {"return_0_F87E"}, {W_REG}, F_V4},
    ...
    {0xF882, {"get_pl_job"}, {R_REG}, F_V2_V4},
    ...
    {0xF886, {"ba_get_place"}, {R_REG, W_REG}, F_V2_V4},

    // Returns (in regB) the battle score of the player specified by regA.
    {0xF887, {"ba_get_score"}, {R_REG, W_REG}, F_V2_V4},
    ...
    {0xF88B, {"send_mail"}, {R_REG, CSTRING}, F_V2_V4 | F_ARGS},

    // Returns the game's major version (2 on DCv2/PC/GCNTE, 3 on GC/Ep3, 4 on Xbox and BB)
    {0xF88C, {"get_game_version"}, {W_REG}, F_V2_V4},
    ...
    {0xF88F, {"set_cmode_grave_rates"}, {{R_REG_SET_FIXED, 20}}, F_V2_V4},
    ...
    // regA <<= regB
    {0xF898, {"shift_left"}, {W_REG, R_REG}, F_V2_V4},

    // regA >>= regB
    {0xF899, {"shift_right"}, {W_REG, R_REG}, F_V2_V4},
    ...
    // zero and not used.
    {0xF8A7, {"set_shrink_size"}, {R_REG, {R_REG_SET_FIXED, 3}}, F_V2_V4},
    ...
    {0xF8AA, {"is_there_grave_message"}, {W_REG}, F_V2_V4},
    ...
    {0xF8AB, {"get_ba_record"}, {{W_REG_SET_FIXED, 7}}, F_V2_V4},
    ...
    //   2 = A rank
    //   3 = S rank
    {0xF8AC, {"get_cmode_prize_rank"}, {W_REG}, F_V2_V4},
    ...
    {0xF8AE, {"party_has_name"}, {W_REG}, F_V2_V4},

    // Returns 1 (in regA) if there is a chat message available (that is, if anyone has sent a chat message in the
    // current game).
    {0xF8AF, {"someone_has_spoken"}, {W_REG}, F_V2_V4},

    // Reads a 1-byte, 2-byte, or 4-byte value from the address (regB/valueB) and places it in regA.
    {0xF8B0, {"read1"}, {W_REG, R_REG}, F_V2},
    {0xF8B0, {"read1"}, {W_REG, I32}, F_V3_V4 | F_ARGS},
    {0xF8B1, {"read2"}, {W_REG, R_REG}, F_V2},
    {0xF8B1, {"read2"}, {W_REG, I32}, F_V3_V4 | F_ARGS},
    {0xF8B2, {"read4"}, {W_REG, R_REG}, F_V2},
    {0xF8B2, {"read4"}, {W_REG, I32}, F_V3_V4 | F_ARGS},
    ...
    // the address in regA.
    {0xF8B3, {"write1"}, {R_REG, R_REG}, F_V2},
    {0xF8B3, {"write1"}, {I32, I32}, F_V3_V4 | F_ARGS},
    {0xF8B4, {"write2"}, {R_REG, R_REG}, F_V2},
    {0xF8B4, {"write2"}, {I32, I32}, F_V3_V4 | F_ARGS},
    {0xF8B5, {"write4"}, {R_REG, R_REG}, F_V2},
    {0xF8B5, {"write4"}, {I32, I32}, F_V3_V4 | F_ARGS},
    ...
    {0xF8BC, {"get_player_guild_card_number"}, {R_REG, W_REG}, F_PC_V2},
    ...
    {0xF8BC, {"set_episode"}, {I32}, F_V3_V4},
    ...
    {0xF8BE, {"create_symbol_chat_monitor"}, {}, F_PC_V2},
    ...
    {0xF8BF, {"save_system_and_gc_files"}, {}, F_PC_V2},
    ...
    {0xF8C0, {"file_dl_req"}, {I32, CSTRING}, F_V3 | F_ARGS},
    {0xF8C0, {"nop_F8C0"}, {I32, CSTRING}, F_V4 | F_ARGS},

    // Returns the status of the download requested with file_dl_req. Return values (in regA):
    //   0 = failed (server sent a D7 command)
    //   1 = pending
    //   2 = complete
    {0xF8C1, {"get_dl_status"}, {W_REG}, F_V3},
    {0xF8C1, {"nop_F8C1"}, {R_REG}, F_V4},
    ...
    {0xF8C3, {"return_0_F8C3"}, {W_REG}, F_XB_V3},
    {0xF8C3, {"nop_F8C3"}, {R_REG}, F_V4},
    ...
    //   regA = client ID
    {0xF8CC, {"set_slot_poison"}, {R_REG}, F_V3_V4},
    {0xF8CD, {"set_slot_paralyze"}, {R_REG}, F_V3_V4},
    {0xF8CE, {"set_slot_shock"}, {R_REG}, F_V3_V4},
    {0xF8CF, {"set_slot_freeze"}, {R_REG}, F_V3_V4},
    {0xF8D0, {"set_slot_slow"}, {R_REG}, F_V3_V4},
    {0xF8D1, {"set_slot_confuse"}, {R_REG}, F_V3_V4},
    {0xF8D2, {"set_slot_shifta"}, {R_REG}, F_V3_V4},
    {0xF8D3, {"set_slot_deband"}, {R_REG}, F_V3_V4},
    {0xF8D4, {"set_slot_jellen"}, {R_REG}, F_V3_V4},
    {0xF8D5, {"set_slot_zalure"}, {R_REG}, F_V3_V4},
    ...
    {0xF8D6, {"fleti_fixed_camera"}, {{R_REG_SET_FIXED, 6}}, F_V3_V4 | F_ARGS},

    // Sets the camera to follow the player at a fixed angle.
    //   valueA = client ID
    //   regsB[0-2] = camera angle (x, y, z as floats)
    {0xF8D7, {"fleti_locked_camera"}, {I32, {R_REG_SET_FIXED, 3}}, F_V3_V4 | F_ARGS},
    ...
    {0xF8E7, {"at_coords_call_ex"}, {{R_REG_SET_FIXED, 5}, W_REG}, F_V3_V4},
    {0xF8E8, {"at_coords_talk_ex"}, {{R_REG_SET_FIXED, 5}, W_REG}, F_V3_V4},
    ...
    {0xF8EC, {"check_npc_straggle_ex", "npc_check_straggle_ex", "col_plinaw_ex"}, {{R_REG_SET_FIXED, 9}, W_REG}, F_V3_V4},

    // Returns 1 if the player is doing certain animations. (TODO: Which ones?)
    //   regA = client ID
    //   regB = returned value
    {0xF8ED, {"animation_check"}, {R_REG, W_REG}, F_V3_V4},
    ...
    {0xF8EE, {"call_image_data"}, {I32, {LABEL16, Arg::DataType::IMAGE_DATA}}, F_V3_V4 | F_ARGS},
    ...
    {0xF8F3, {"particle2"}, {{R_REG_SET_FIXED, 3}, I32, FLOAT32}, F_V3_V4 | F_ARGS},

    // Converts the integer in regB into a float in regA.
    {0xF901, {"dec2float"}, {W_REG, R_REG}, F_V3_V4},

    // Converts the float in regB into an integer in regA.
    {0xF902, {"float2dec"}, {W_REG, R_REG}, F_V3_V4},
    ...
    {0xF908, {"fadd"}, {W_REG, R_REG}, F_V3_V4},
    {0xF909, {"faddi"}, {W_REG, FLOAT32}, F_V3_V4},

    // regA -= regB (or valueB), as floats
    {0xF90A, {"fsub"}, {W_REG, R_REG}, F_V3_V4},
    {0xF90B, {"fsubi"}, {W_REG, FLOAT32}, F_V3_V4},

    // regA *= regB (or valueB), as floats
    {0xF90C, {"fmul"}, {W_REG, R_REG}, F_V3_V4},
    {0xF90D, {"fmuli"}, {W_REG, FLOAT32}, F_V3_V4},

    // regA /= regB (or valueB), as floats
    {0xF90E, {"fdiv"}, {W_REG, R_REG}, F_V3_V4},
    {0xF90F, {"fdivi"}, {W_REG, FLOAT32}, F_V3_V4},
    ...
    {0xF91D, {"get_time_played"}, {W_REG}, F_V3_V4},

    // Returns the number of Guild Cards saved to the Guild Card file.
    {0xF91E, {"get_guildcard_total"}, {W_REG}, F_V3_V4},
    ...
    {0xF91F, {"get_slot_meseta"}, {{W_REG_SET_FIXED, 2}}, F_V3_V4},

    // Returns a player's level.
    //   valueA = client ID
    //   regB = returned level
    {0xF920, {"get_player_level"}, {CLIENT_ID, W_REG}, F_V3_V4 | F_ARGS},
    ...
    {0xF924, {"get_coord_player_detect"}, {{R_REG_SET_FIXED, 3}, {W_REG_SET_FIXED, 4}}, F_V3_V4},
    ...
    {0xF929, {"nop_F929"}, {CSTRING}, F_XB_V3 | F_V4 | F_ARGS},
    ...
    {0xF92F, {"nop_F92F"}, {I32, I32}, F_XB_V3 | F_V4 | F_ARGS},
    ...
    {0xF933, {"nop_F933"}, {{R_REG_SET_FIXED, 7}}, F_V4},
    ...
    //   strH = message
    {0xF934, {"scroll_text"}, {I32, I32, I32, I32, I32, FLOAT32, W_REG, CSTRING}, F_V3_V4 | F_ARGS},
    ...
    {0xF93A, {"get_item_info"}, {ITEM_ID, {W_REG_SET_FIXED, 12}}, F_V3_V4 | F_ARGS},
    ...
    {0xF941, {"get_guildcard_num"}, {CLIENT_ID, W_REG}, F_V3_V4 | F_ARGS},
    ...
    {0xF945, {"initial_floor"}, {I32}, F_V3_V4 | F_ARGS},

    // Computes the sine, cosine, or tangent of the input angle. Angles are in the range [0, 65535].
    //   regA = result value
    //   valueB = input angle
    {0xF946, {"sin"}, {W_REG, I32}, F_V3_V4 | F_ARGS},
    {0xF947, {"cos"}, {W_REG, I32}, F_V3_V4 | F_ARGS},
    {0xF948, {"tan"}, {W_REG, I32}, F_V3_V4 | F_ARGS},
    ...
    {0xF94D, {"has_ep3_save_file"}, {W_REG}, F_GC_V3 | F_ARGS},
    ...
    {0xF94D, {"unknown_F94D"}, {I32, W_REG}, F_XB_V3 | F_ARGS},
    ...
    // necessary header fields without doing any fancy static analysis.
    //   valueA = floor number
    //   valueB = area number
    ...
    {0xF95A, {"bb_is_ep4_boss_dying"}, {W_REG}, F_V4},
    ...
    while (!label_r.eof()) {
      ...
      try {
        ...
        if (def == nullptr) {
          ...
          const char* op_name = use_qedit_names ? def->names.back() : def->names.front();
