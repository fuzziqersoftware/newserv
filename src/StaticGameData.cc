...
const unordered_map<uint32_t, const char*> name_for_primary_identifier({
  // Weapons (00xxxx)
  {0x000100, "Saber"},
  {0x000101, "Brand"},
  {0x000102, "Buster"},
  ...
  {0x030101, "Difluid"},
  {0x030102, "Trifluid"},
  {0x030200, "<TECH-DISK>"}, // Special-cased in name_for_item
...
string name_for_item(const ItemData& item) {
  ...
  if ((primary_identifier & 0xFFFFFF00) == 0x00030200) {
    ...
    try {
      ret_tokens.emplace_back(name_for_primary_identifier.at(primary_identifier));
    } catch (const out_of_range&) {
