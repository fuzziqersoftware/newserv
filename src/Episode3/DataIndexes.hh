...
enum class CardRank : uint8_t {
  ...
  R4 = 0x0A,
  SS = 0x0B,
  // Cards with the D1 or D2 ranks are considered never usable by the player, and are automatically removed from player
  // decks before battle and when loading the deckbuilder. Cards with the D1 rank appear in the deck builder but are
  // grayed out (and cannot be added to decks); cards with the D2 rank don't appear in the deckbuilder at all.
  D1 = 0x0C,
  D2 = 0x0D,
  // The D3 rank is referenced in a few places, including the function that determines whether or not a card can appear
  // in post-battle draws, and the function that determines whether a card should appear in the deckbuilder. In these
  // cases, it prevents the card from appearing.
  D3 = 0x0E,
};
...
enum class ConditionType : uint8_t {
  ...
  DEF_DISABLE_BY_COST = 0x2A, // Disable use of defense cards costing between (N / 10) and (N % 10) points, inclusive
  ...
  REBORN = 0x50, // If any card of type N is on the field, this card goes back to the hand when destroyed
  ...
  PARRY = 0x5D, // Forward attack to random FC within one tile of original target, except attacker and original target
...
struct CardDefinition {
  ...
  struct Effect {
    ...
    // For ConditionTypes that need it, expr specifies "how much". (For those that don't, expr may be blank.) The value
    // may contain tokens that refer to stats from the current battle (see description_for_expr_token) and operators to
    // perform basic computations on them. Operators are evaluated left-to-right in the expression, and there are no
    // operator precedence rules; for example, the expression "4+4//2" results in 4, not 6.
    ...
    // name_index specifies which string from TextEnglish.pr2 is shown next to the card when it is attacking or
    // defending. Zero in this field means no string is shown for this ability.
  ...
  // The list of card definitions ends with a sentinel definition that isn't a real card, but instead has a negative
  // number in the type field here.
  /* 0044 */ CardType type;
  ...
  // In the definitions file, only .code is populated here; .decode_code() must be called to fill in .type and .stat
  ...
  // The card's attack range is defined in a somewhat odd format here. Each field in this array corresponds to a single
  // row of the range, and every fourth bit, starting with bit 15, corresponds to a tile in that row. The rest of the
  // bits are ignored, except in two special cases described below. For example, Ohgun's range is:
  //   [0] = 0x00000000 => -----
  //   [1] = 0x00001110 => -***-
  //   [2] = 0x00001110 => -***-
  //   [3] = 0x00000000 => -----
  ...
  // 1. If all six values in the range array are 0x000FFFFF, then the card's range is the entire field.
  // 2. If the cell corresponding to the card itself ((range[4] >> 8) & 0x0F) is not zero, then the rest of the range
  //    array is ignored and the card's range comes from a fixed set of ranges instead. See decode_range() for details.
  ...
  // This field is 1 if the card cannot move by itself. Item cards hare 1 here because they cannot move on their own
  // and automatically move along with their SC instead. Generally only SCs and creatures have 0 here.
  /* 008E */ uint8_t cannot_move;
  // This field is 1 if the card cannot take part in an attack. Unlike cannot_move, cards that cannot attack on their
  // own but can take part in an attack (such as action cards) have 0 here. Most shields, mags, defense actions, and
  // assist cards have 1 here.
  /* 008F */ uint8_t cannot_attack;
  /* 0090 */ uint8_t unused3;
  // If cannot_drop is 1, this card can't appear in post-battle rewards and is considered unobtainable by players, so
  // the game will remove it from the player's collection if they have any copies of it. A value of 1 here also
  // prevents the card from being used as a God Whim random assist.
  /* 0091 */ uint8_t cannot_drop;
  // This criterion code specifies who can use the card, and when it can be used. This specifies which Hero-side SCs
  // can use which items, for example, and when action cards can be played (when SC or FC is attacking, on self or
  // ally, etc.).
  /* 0092 */ CriterionCode usable_criterion;
  /* 0093 */ CardRank rank;
  /* 0094 */ be_uint16_t unused4;
  // The card class is used for checking attributes (e.g. item types). It's stored big-endian here, so there's a helper
  // function (card_class()) that returns a usable CardClass enum value.
  /* 0096 */ be_uint16_t be_card_class;

  // If this card is an assist card, this field controls how COM players handle playing it. This field is ignored for
  // non-assist cards. This integer encodes the following fields:
  // - assist_ai_params % 100 (that is, the two lowest decimal places) appears to specify the effect, though a few
  //   unrelated cards share values in this field. It's not yet known how exactly this is used by the COM logic.
  // - (assist_ai_params / 100) % 10 specifies the priority. It appears the COM logic always chooses the assist card
  //   with the highest value in this field if there are multiple cards to consider.
  // - (assist_ai_params / 1000) % 10 specifies on whom the assist card may be played (0 = any player, 1 = self, 2 =
  //   self or ally, 3 = enemy only).
  /* 0098 */ be_uint16_t assist_ai_params;
  // Most cards in the official definitions file have the same value stored in unused5 as in assist_ai_params. Unlike
  // assist_ai_params, unused5 does not appear to be used anywhere.
  /* 009A */ be_uint16_t unused5;

  // The card drop rates control how likely the card is to appear in a standard post-battle random draw. How this works
  // is fairly complex and is explained below in detail. Before any of that logic, this card can never drop and no card
  // can transform into this card if any of the following are true:
  // - type is SC_HUNTERS or SC_ARKZ
  // - card_class is BOSS_ATTACK_ACTION (0x23) or BOSS_TECH (0x24)
  // - rank is E, D1, or D2
  // - cannot_drop is 1 (specifically 1; other nonzero values don't prevent the card from appearing in draws)
  // If none of these conditions apply, the logic below is used.
  //
  // Drop rates are integers which encode the following data:
  // - rate % 10 (that is, the lowest decimal place) specifies the required game mode. 0 means any mode, 1 means
  //   offline story mode, 2 means 1P free battle, 3 means 2P+ free battle (specifically, PvP - two humans vs. two COMs
  //   counts as 1P free battle), 4 means online mode, 5 means tournament. Some cards have this field set to 6, which
  //   isn't a valid game mode; it seems Sega used this as a way to make sure the drop rate never applies.
  ...
  //
  // When determining which cards to drop, the game first checks the drop rate fields on all cards. For each drop rate
  ...
  // These values are all relative to other values in the same row. For example, if your character is in level class 1,
  // you'll get cards of rarity class 0 about 80% of the time, cards of rarity class 1 about 20% of the time, and cards
  // of rarity class 2 about 0.5% of the time. (The actual probabilities are 8000/10050, 2000/10050, and 50/10050.)
  //
  // When choosing the contents of the four card packs after a battle, the game first chooses how many cards to give
  // the player based on the end-of-battle rank (9 for S, 8 for A+/A, 7 for B+/B, 6 for C+/C, 5 for D+/D/E, 2 if the
  // player lost). It then decides the number of "restricted" cards; if the player is getting 6 or more cards, there
  // are 2 restricted cards per pack, otherwise there is only one. The restricted cards are required to be a certain
  // type in each pack except the black pack:
  ...
  // R2 =>                                30   60    5
  // R1 =>                                    900  100    1
  // For example, when an N2 card transforms, there is a 60% chance to become R4, a 30% chance to become R3, and a 10%
  // chance to become R2. When an R1 card transforms, there is a 900/1001 chance of becoming another R1, a 100/1001
  // chance of becoming an S, and a 1/1001 chance of becoming an SS.
  //
  // Once a rank is chosen, the game puts all possible cards into buckets based on how many of that card the player
  // already has, then chooses a random card out of bucket 0, then bucket 1, etc. all the way up to bucket 49 (or 2 if
  // the final rank is S or SS). The first drawn card that has the final rank is the card that the original card
  // transforms into. Notably, this logic means that cards are more likely to transform into cards that the player
  // doesn't already have, or only has few copies of. Also notably, it is impossible for a card to transform into
  // another card that the player already has 50 or more copies of, or an S or SS card that the player already has 3
  // copies of.
  //
  // One curiosity about the above procedure is that the buckets can only hold 400 cards each for the N ranks, 300 each
  // for the R ranks, and 100 each for S and SS. It is possible for one bucket to overflow into the next, or to
  // overflow out of bounds and cause memory corruption, if there are (for example) more than 400 cards that have ranks
  // N1-N4, and the player has 99 of all of them.
  //
  // Remember the drop rates from back in the second paragraph of this enormous comment? That's what this array stores.
  ...
  // These effects modify the card's behavior in various situations. Only entries where effect_num != 0 are used.
...
struct DeckDefinition {
  ...
  // List of card IDs. The card count is the number of nonzero entries here before a zero entry (or 50 if no entries
  // are nonzero). The first card ID is the SC card, which the game implicitly subtracts from the limit - so a valid
  // deck should actually have 31 cards in it.
...
struct PlayerConfig {
  // The game splits this internally into two structures. The first column of offsets is relative to the start of the
  // first structure; the second column is relative to the start of the second structure.
  ...
  // place_counts[0] and [1] from this field are added to the player's win and loss count respectively when they're
  // shown in the status menu. However, these values start at 0 and never seem to be modified. Perhaps in an earlier
  // version, this was the offline records structure, but they later decided to just count online and offline records
  // together in the main records structure and didn't remove the codepath that reads from this.
  /* 0138:---- */ PlayerRecordsBattleBE unused_offline_records;
  /* 0150:---- */ parray<uint8_t, 4> unknown_a4;
  // The PlayerDataSegment structure begins here. In newserv, we combine this structure into PlayerConfig since the two
  // are always used together on the server side.
  /* 0154:0000 */ uint8_t is_encrypted;
  /* 0155:0001 */ uint8_t basis;
  /* 0156:0002 */ parray<uint8_t, 2> unused;
  // The following fields (here through the beginning of decks) are encrypted using the trivial algorithm, with the
  // basis specified above, if is_encrypted is equal to 1.
  // It appears the card counts field in this structure is actually 1000 (0x3E8) bytes long, even though in every other
  // place the counts array appears it's 0x2F0 bytes long. They presumably did this because of the checksum logic.
  /* 0158:0004 */ parray<uint8_t, 1000> card_counts;
  // These appear to be an attempt at checksumming the card counts array, but the algorithm doesn't cover the entire
  // array and instead reads from later parts of this structure. This appears to be due to a copy/paste error in the
  // original code. The algorithm sums card_counts [0] through [19] and puts the result in card_count_checksums[0],
  // then sums card counts [50] through [69] and puts the result in card_count_checksums[1], etc. Presumably they
  // intended to use 20 as the stride instead of 50, which would have exactly covered the entire card_counts array.
  /* 0540:03EC */ parray<be_uint16_t, 50> card_count_checksums;
  // These 64-bit integers encode information about when rare cards (those with ranks S, SS, E, or D2) were obtained.
  // Each integer contains the following bit fields:
  //   ???????? PPPPPPPP PPPPVVVV VVVVVVVV VVVVVVVV VVVVVVVV VVVVVVVV VVVVVVVV
  // The meaning of the high byte is unknown, but it is not used by the decoding function. P is a prime number between
  // 1009 (0x3F1) and 2039 (0x7F7), inclusive. V is a 44-bit integer that, when modulated by P, yields the card ID
  // (that is, V % P == card_id). When a non-rare card is obtained or lost, the game just increments or decrements the
  // value in the card_counts array above, but when a rare card is obtained or lost, the game adds or removes a token
  // in rare_tokens and recomputes the count for that card by scanning and decoding all rare tokens. It then writes
  // that count to card_counts. This seems to be an anti-cheating measure specifically targeted at memory editing - the
  // server could verify that the count in card_counts is correct for rare cards by counting the valid tokens in this
  // array. (Sega seemed fairly concerned with memory editing in general in this game, since the card counts array is
  // encrypted in memory most of the time, and they went out of their way to ensure the game uses an area of memory
  // that almost no other game uses, which is also used by the Action Replay.)
  ...
  // These two arrays are updated when a battle is started (via a 6xB4x05 command). The client adds the opposing
  // players' info to ths first two entries in recent_human_opponents if the opponents are human. (The existing entries
  // are always moved back by two slots, but if one or both opponents are not humans, one or both of the newly-vacated
  // slots is not filled in.) Both arrays have the most recent entries at the beginning.
  ...
  // In a certain situation, unknown_t3 is set to init_timestamp plus a multiple of two weeks (1209600 seconds).
  // unknown_t3 appears never to be used for anything, though.
...
struct Rules {
  // When this structure is used in a map/quest definition, FF in any of these fields means the user is allowed to
  // override it. Any non-FF fields are fixed for the map/quest and cannot be overridden.

  // The overall time limit is specified in increments of 5 minutes; that is, 1 means 5 minutes, 2 means 10 minutes,
  // etc. 0 means no overall time limit.
  ...
  // NOTE: The following fields are unused in PSO's implementation, but newserv uses them to implement extended rules.
...
struct RulesTrial {
  ...
  /* 02 */ AllowedCards allowed_cards = AllowedCards::ALL;
  // In NTE, the dice behave differently than in non-NTE. A zero in either of these fields means the corresponding die
  // is random in the range [1, 6]; any nonzero value means that die will always take that value.
...
struct MapList {
  ...
  struct Entry {
    // The fields in this structure have the same meanings as the corresponding fields in MapDefinition.
    /* 0000 */ be_uint16_t map_x;
    /* 0002 */ be_uint16_t map_y;
    /* 0004 */ be_uint16_t environment_number;
    /* 0006 */ be_uint16_t map_number;
    // Text offsets are from the beginning of the strings block after all map entries (that is, add strings_offset to
    // them to get the string offset)
...
struct OverlayState {
  // In the tiles array, the high 4 bits are the tile type, and the low 4 bits are the subtype. The values are:
  //   10-1F: blocked by rock (as if the corresponding map_tiles value was 00; low 4 bits specify rotation in
  //     increments of 1/4 turn)
  //   20-2F: blocked by fence (as if the corresponding map_tiles value was 00; low 4 bits specify rotation in
  //     increments of 1/4 turn)
  ...
  //   40-4F: traps on NTE
  //   40-44: traps on non-NTE (there may be up to 8 of each type, and one of each is chosen to be a real trap at
  //     battle start); the trap types are:
  ...
  //   50: blocked by metal box (appears as teal cube in preview; behaves like 10 and 20 in game)
  // Any other value here will behave like 00 (no special tile behavior).
  parray<parray<uint8_t, 0x10>, 0x10> tiles;

  // This field appears to be unused in both NTE and the final version. Perhaps it had meaning in a pre-NTE version.
  ...
  // This specifies the assist card IDs that each trap value (40-4F) will set when triggered. This only has an effect
  // on NTE; on non-NTE, this is unused and a fixed set of assist cards is used instead. (On newserv, the set of used
  // assist cards can be overridden in the server configuration.)
...
struct MapDefinition { // .mnmd format; also the format of (decompressed) quests
  // If tag is not 0x00000100, the game considers the map to be corrupt in offline mode and will delete it (if it's a
  // download quest). The tag field doesn't seem to have any other use. In Trial Edition, download quests are expected
  // to have 0x96 here instead.
  /* 0000 */ be_uint32_t tag;

  /* 0004 */ be_uint32_t map_number; // Must be unique across all maps

  // The maximum map size is 16 tiles in either dimension, since the various tiles arrays below are fixed sizes.
  /* 0008 */ uint8_t width;
  /* 0009 */ uint8_t height;
  ...
  // Environment numbers above 12 are replaced with 0B (Cyber) if specified in a map definition. The following
  // environment numbers are used internally by the game for various functions, but cannot be specified in maps:
  ...
  //   02 = team A start (1v1) (can be redefined by start_tile_definitions)
  //   03, 04 = team A start (2v2) (can be redefined by start_tile_definitions)
  //   06, 07 = team B start (2v2) (can be redefined by start_tile_definitions)
  //   08 = team B start (1v1) (can be redefined by start_tile_definitions)
  // Note that the game displays the map reversed vertically in the preview window. For example, player 1 is on team A,
  // which usually starts at the top of the map as defined in this struct, or at the bottom as shown in the preview.
  /* 000C */ parray<parray<uint8_t, 0x10>, 0x10> map_tiles;

  // The start_tile_definitions field is a list of 6 bytes for each team. The low 6 bits of each byte match the
  // starting location for the relevant player in map_tiles; the high 2 bits are the player's initial facing direction.
  ...
  struct CameraSpec {
    ...
    // It appears that the camera always aligns its +Y raster axis with +Y in the virtual world. If the focus point is
    // directly beneath the camera point, the logic for deciding which direction should be "up" from the camera's
    // perspective can get confused and jitter back and forth as the camera moves into position.
    /* 30 */ be_float focus_x;
    /* 34 */ be_float focus_y;
    /* 38 */ be_float focus_z;
  ...
  // This array specifies the camera zone maps. A camera zone map is a subset of the main map (specified in map_tiles).
  // Tiles that are part of each camera zone are 1 in these arrays; all other tiles are 0. The game evaluates each
  // camera zone in order; if all SCs and FCs are within a particular camera zone, then the corresponding camera
  // location is used as the default camera location. If the player doesn't move the camera with the C stick, then the
  // camera zones are evaluated continuously during the battle, and the camera will move to focus on the part of the
  // field where the SCs/FCs are. (Or, more accurately, where the corresponding entry in camera_zone_specs says to
  // focus.) camera_zone_maps is indexed as [team_id][camera_zone_num][y][x]; camera_zone_specs is indexed as
  // [team_id][camera_zone_num]. Unused entries (beyond num_camera_zones) in both arrays should be filled with 0xFF.
  ...
  // These camera specs are used in the Move phase, when the player has chosen an SC or FC to move, or when the player
  // presses Start/Z. Normally these are defined such that the camera is placed high above the map, giving an overhead
  // view of the entire playfield. This is indexed as [???][team_id] (it is not yet known what the major index is).
  /* 1AB8 */ parray<parray<CameraSpec, 2>, 3> overview_specs;

  // This specifies the locations of blocked tiles, teleporters, and traps. See comments in OverlayState for details.
  ...
  // These fields describe where the map cursor on the preview screen should scroll to.
  ...
  // This array specifies which set of predefined AI parameters (in aiprm.dat) to use for each NPC. If a value in this
  // array is -1 (FFFFFFFF), then the corresponding NPC's AI parameters are defined in the AIParams structure above.
  // The names of the AI parameter sets defined in aiprm.dat are:
  ...
  // Presumably 0A is meant to be Stella and 1D is meant to be Amplum Umbra, but they forgot to change the names.
  /* 2434 */ parray<be_int32_t, 3> npc_ai_params_entry_index;

  // before_message appears before the battle if it's not blank. after_message appears after the battle if it's not
  // blank. dispatch_message appears right before the player chooses a deck if it's not blank; usually it says
  ...
  struct DialogueSet {
    // Dialogue sets specify lines that COMs can say at certain points during the battle. They only apply to COMs which
    // are defined as NPCs in the map definition; human players and COMs chosen by humans never say these lines.

    // when specifies the situation in which this dialogue set activates. The values 0000-000C (inclusive) can be used
    // here, or FFFF if the entire dialogue set is unused. The known values are:
    ...
    // If the dialogue set activates, the game randomly chooses one of these strings, excluding any that are empty or
  ...
  // There are up to 0x10 of these per valid NPC, but only the first 13 of them are used, since each one must have a
  // unique value for .when and the values there can only be 0-12.
  /* 28F0 */ parray<parray<DialogueSet, 0x10>, 3> dialogue_sets;

  // These card IDs are always given to the player when they win a battle on this map. Unused entries should be set to
  // FFFF. Cards in this array are ignored if they have any of these features (in the card definition):
  // - type is HUNTERS_SC or ARKZ_SC
  // - card_class is BOSS_ATTACK_ACTION or BOSS_TECH
  // - rank is D1, D2, or D3
  // - cannot_drop is 1 (specifically 1; other values don't prevent cards from appearing)
  /* 59B0 */ parray<be_uint16_t, 0x10> reward_card_ids;

  // These fields are used when determining which cards to drop after the battle is complete. If either is negative,
  // the player's CLv is used instead.
  /* 59D0 */ be_int32_t win_level_override;
  /* 59D4 */ be_int32_t loss_level_override;

  // The field offsets specify where the battlefield should appear relative to the center of the environment. The size
  // of one tile on the field is 25 units in these fields.
  /* 59D8 */ be_int16_t field_offset_x;
  /* 59DA */ be_int16_t field_offset_y;

  // map_category specifies where the map should appear in the maps menu. If this is 0, 1, or 2, the map appears in the
  // Quest section; otherwise, it appears in the Free Battle section instead. It's not known if this controls anything
  // else, or what the difference is in behavior between 0, 1, and 2.
  /* 59DC */ uint8_t map_category;

  // This field determines block graphics to be used in the Cyber environment. There are 10 block types (0-9); if this
  // value is > 9, type 0 is used. This field has no effect in Ep3 NTE, even though there are 6 different block texture
  // files on the NTE disc.
  /* 59DD */ uint8_t cyber_block_type;

  /* 59DE */ be_uint16_t unknown_a11;

  // This array specifies which SC characters can't participate in the quest (that is, the player is not allowed to
  // choose decks with these SC cards). The values in this array don't match the SC card IDs, however:
  ...
  // These values normally can't be used by the player, but are recognized internally by the game:
  ...
  // This function throws runtime_error if the passed-in map is not semantically equivalent to *this. Semantic
  // equivalence means all fields that affect gameplay and visuals are equivalent, but dialogue, names, and description
  // text may differ.
...
struct MapDefinitionTrial {
  // This is the format of Episode 3 Trial Edition maps. See the comments in MapDefinition for what each field means.
  ...
  // TODO: This field may contain some version of unavailable_sc_cards and/or entry_states from MapDefinition, but the
...
private:
  // The compressed map lists are generated on demand; these are indexed as [language][num_players]
